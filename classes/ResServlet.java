...
public class ResServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {	
		...
		String sql = "SELECT * FROM thread_sample3 Where threadID="+threadID;
		thread = _threadDB.singleSelect(sql);

        ResDBAccess _resDB = new ResDBAccess();
    	sql = "SELECT * FROM res_sample3 Where threadID="+threadID+"ORDER BY resID desc";
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {	
		...
		String _resContent = req.getParameter("resContent");
		...
		//resIDの取得
		String sql = "SELECT COUNT(*) FROM res_sample3 WHERE threadID = "+threadID;
		int count = _resDB.resCount(sql);
		int resID = count + 1;

		String resContent = _resContent.replaceAll("\n", "<br>");
		
		if(check.equals(resContent)){
		}
		else{
		sql = "INSERT INTO res_sample3 VALUES("+threadID+", "+resID+", '"+resName+"', '"+resContent+"', default)";
		_threadDB.insert(sql);
		}

		sql = "SELECT * FROM thread_sample3 Where threadID = "+threadID;
		thread = _threadDB.singleSelect(sql);
